    ...
    extends TestCase
{
        ...
        throws Exception
    {
        ...
        final URL configMetamodelUrl = SpringTransformationTest.class.getResource("/ConfigurationMetamodel.xml");
        ...
        Model[] metamodels = new Model[] {umlMetamodel, configMetamodel, springMetamodel};
    ...
    {
        // - first registry the "languageMappingsUri" in the spring
        //   namespace registry
        NamespaceRegistry registry = new NamespaceRegistry();
        registry.setName("spring");
        final PropertyDefinition definition = new PropertyDefinition();
        definition.setName(UMLMetafacadeProperties.LANGUAGE_MAPPINGS_URI);
        definition.setRequired(true);
        registry.addPropertyDefinition(definition);
        Namespaces.instance().addRegistry(registry);

        // find the XML file with the test configuration for AndroMDA
        URL uri = SpringTransformationTest.class.getResource("configuration.xml");
        assertNotNull(uri);
        Configuration configuration = Configuration.getInstance(uri);

        // - need to add the mappings search location so that 'Java' mappings are correctly found
        URL mappingsUri = SpringTransformationTest.class.getResource("JavaMappings.xml");
        assertNotNull(mappingsUri);
        configuration.addMappingsSearchLocation(mappingsUri.toString());
