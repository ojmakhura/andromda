...
/**
 * <p>
 * Creates and returns Objects based on a set of Apache Digester rules in a
 * consistent manner, providing validation in the process.
 * </p>
 * <p>
 * This XML object factory allows us to define a consistent/clean of configuring
 * java objects from XML configuration files (i.e. it uses the class name of the
 * java object to find what rule file and what XSD file to use). It also allows
 * us to define a consistent way in which schema validation is performed.
 * </p>
 * <p>
 * It seperates each concern into one file, for example: to configure and
 * perform validation on the MetafacadeMappings class, we need 3 files 1.) the
 * java object (MetafacadeMappings.java), 2.) the rules file which tells the
 * apache digester how to populate the java object from the XML configuration
 * file (MetafacadeMappings-Rules.xml), and 3.) the XSD schema validation file
 * (MetafacadeMappings.xsd). Note that each file is based on the name of the
 * java object: 'java object name'.xsd and 'java object name'-Rules.xml'. After
 * you have these three files then you just need to call the method
 * #getInstance(java.net.URL objectClass) in this class from the java object you
 * want to configure. This keeps the dependency to digester (or whatever XML
 * configuration tool we are using at the time) to this single file.
 * </p>
 * <p>
 * In order to add/modify an existing element/attribute in your configuration
 * file, first make the modification in your java object, then modify it's rules
 * file to instruct the digester on how to configure your new attribute/method
 * in the java object, and then modify your XSD file to provide correct
 * validation for this new method/attribute. Please see the
 * org.andromda.core.metafacade.MetafacadeMappings* files for an example on how
 * to do this.
 * </p>
 * 
 * @author Chad Brandon
...
public class XmlObjectFactory
{
    ...
    /**
     * Whether or not validation should be turned on by default when using this
     * factory to load XML configuration files.
     */
    private static boolean defaultValidating = true;

    /**
     * Cache containing XmlObjectFactory instances which have already been
     * configured for given objectRulesXml
     */
    private static Map factoryCache = new HashMap();

    /**
     * Creates an instance of this XmlObjectFactory with the given
     * <code>objectRulesXml</code>
     * 
        ...
        URL objectRulesXml)
    {
        ...
        if (logger.isDebugEnabled())
            logger.debug("performing " + methodName + " with objectRulesXml '"
                + objectRulesXml + "'");
    ...
    /**
     * Gets an instance of this XmlObjectFactory using the digester rules
     * belonging to the <code>objectClass</code>.
     * 
     * @param objectClass the Class of the object from which to configure this
     *        factory.
    ...
    public static XmlObjectFactory getInstance(Class objectClass)
    {
        ...
        XmlObjectFactory factory = (XmlObjectFactory)factoryCache
            .get(objectClass);
        if (factory == null)
        {
    ...
    /**
     * Allows us to set default validation to true/false for all instances of
     * objects instantiated by this factory. This is necessary in some cases
     * where the underlying parser doesn't support schema validation (such as
     * when performing JUnit tests)
     * 
    ...
    {
        return this.getObject(objectXml != null ? ResourceUtils
            .getContents(objectXml) : null);
    }
    ...
    public Object getObject(String objectXml)
    {
        ...
        if (logger.isDebugEnabled())
            logger.debug("performing " + methodName + " with objectXml '"
                + objectXml + "'");
