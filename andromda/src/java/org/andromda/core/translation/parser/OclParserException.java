...
/**
 * Retrieves information from the OCL parser exceptions in a more
...
public class OclParserException extends RuntimeException {
	 ...
	 *
	 * <p>Error line and column are stored in {@link #errorLine} and
	 * {@link #errorCol} so that they can be retrieved using
	 * {@link #getErrorLine} and {@link #getErrorCol}. The detail message without
	 * the position information is stored in {@link #detailMessage}</p>
	...
	private void extractErrorPosition(String sDetailMessage) {
		...
		if (sDetailMessage.charAt(0) == '[') {
			...
			StringTokenizer st =
				new StringTokenizer(sDetailMessage.substring(1), ",]");
