...
public class TranslationUtils
{
    ...
    /**
     * <p>
     * <code>TranslationUtils</code> instances should NOT be constructed in
     * standard programming. Instead, the class should be used as
     * <code>TranslationUtils.replacePattern(" some pattern ");</code>.
     * </p>
     * <p>
     ...
     * like so --> "{pattern}" every time it occurs in the string.
     * 
    ...
    /**
     * Searches for and replaces the specified pattern with braces around it,
     * like so --> "{pattern}" the first time it occurs in the string
     * 
     * @param string the string to to perform replacement on.
     * @param pattern the pattern to find
     * @param replaceWith the pattern to place the existing one with.
        ...
        String replaceWith)
    {
        ...
        if (string != null)
        {
            ExceptionUtils.checkNull(methodName, "pattern", pattern);
            ExceptionUtils.checkNull(methodName, "replaceWith", replaceWith);
            string = StringUtils.replaceOnce(
                string,
                "{" + pattern + "}",
                replaceWith);
        }
    ...
    /**
     * Returns true if the specified pattern with braces around it, like so -->
     * "{pattern}" exists in the string.
     * 
     * @param string the string to to perform replacement on.
     * @param pattern the pattern to find
    ...
    /**
     * Calls the object's toString method and trims the value. Returns and empty
     * string if null is given.
     * 
    ...
    /**
     * Calls the object's toString method and deletes any whitespace from the
     * value. Returns and empty string if null is given.
     * 
    ...
    /**
     * Retrieves the "starting" property name from one that is nested, for
     * example, will return ' <name1>' from the string --> <name1>. <name2>.
     * <name3>. If the property isn't nested, then just return the name that is
     * passed in.
     * 
    ...
    /**
     * Removes any extra whitepace --> does not remove the spaces between the
     * words. Only removes tabs and newline characters. This is to allow
     * everything to be on one line while keeping the spaces between words.
     * 
     * @param string
    ...
    /**
     * Just retriieves properties from a bean, but gives a more informational
     * error when the property can't be retrieved, it also cleans the resulting
     * property from any excess white space
     * 
    ...
    /**
     * Returns true/false on whether or not the give <code>bean</code> has the
    ...
    {
        return TranslationUtils.trimToEmpty(TranslationUtils.getProperty(
            bean,
            property));
    }
