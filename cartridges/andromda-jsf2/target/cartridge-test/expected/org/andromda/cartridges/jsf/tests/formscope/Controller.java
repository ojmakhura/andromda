...
public abstract class Controller
    ...
    {
        // Resets all the "isSet" flags on the forms to false.
        this.getAPageSubmitForm().resetIsSetFlags();
        this.getFormScopeForm().resetIsSetFlags();
    }

    /**
     * Retrieves the {@link APageSubmitFormImpl} form instance (normally you wont't
     * need to call this method explicitly, however this is here for times when you need to access the
     * <em>formScopeAPageSubmitForm</em> instance outside of its assigned controller operation).
     * @return APageSubmitFormImpl
     */
    protected APageSubmitFormImpl getAPageSubmitForm()
    {
        ...
        final UIForm uiForm = this.findForm(this.getContext().getViewRoot(), "formScopeAPageSubmitForm");
        this.populateComponentInputs(uiForm);
        return (APageSubmitFormImpl)this.resolveVariable("formScopeAPageSubmitForm");
    }

    /**
     * 
     * This method is called when 'submit' is triggered in the view 'A Page'.
     * It can be safely overridden in descendant classes.
APageSubmitFormImpl)
     * @param form
     */
    protected void _aPage_submit(APageSubmitFormImpl form)
    {
     ...
     * @return aPageSubmit
     */
    public String aPageSubmit()
    {
        return aPageSubmit((FacesEvent)null);
    }

    /**
     * @param event
     */
    public void aPageSubmit(final ActionEvent event)
    {
        this.aPageSubmit((FacesEvent)event);
    }

    /**
     * @param event
     */
    public void aPageSubmit(final ValueChangeEvent event)
    {
        this.aPageSubmit((FacesEvent)event);
    }
    ...
    public String aPageSubmit(final FacesEvent event)
    {
        Controller controller =
            (Controller)this.resolveVariable(
                "controller");
        return controller.formScope();
    }

    /**
     * Retrieves the {@link FormScopeFormImpl} form instance (normally you wont't
     * need to call this method explicitly, however this is here for times when you need to access the
     * <em>formScopeFormScopeForm</em> instance outside of its assigned controller operation).
     * @return FormScopeFormImpl
     */
    protected FormScopeFormImpl getFormScopeForm()
    {
        // - we do this in the case a button that submitted the form was set to immediate (this should be removed
        //   when we found a better way to handle this).
        final UIForm uiForm = this.findForm(this.getContext().getViewRoot(), "formScopeFormScopeForm");
        this.populateComponentInputs(uiForm);
        return (FormScopeFormImpl)this.resolveVariable("formScopeFormScopeForm");
    }

    /**
     * This method is called when the use case 'FormScope' starts.
     * It can be safely overridden in descendant classes.
FormScopeFormImpl)
     * @param form
     */
    protected void _formScope_started(FormScopeFormImpl form)
    {
        //this method can be overridden
    }
     ...
     * @return formScope
     */
    public String formScope()
    {
        return formScope((FacesEvent)null);
    }

    /**
     * @param event
     */
    public void formScope(final ActionEvent event)
    {
        this.formScope((FacesEvent)event);
    }

    /**
     * @param event
     */
    public void formScope(final ValueChangeEvent event)
    {
        this.formScope((FacesEvent)event);
    }
     ...
     */
    @SuppressWarnings({ "unchecked", "rawtypes" })
    public String formScope(final FacesEvent event)
    {
        String forward = null;
        final Object currentForm = this.resolveVariable("form");
        try
        {
            final FormScopeFormImpl form =
                this.getFormScopeForm();
            form.setEvent(event);
            ...
            _formScope_started(form);

            forward = _sendPageVariable(form);
            ...
            if (event != null)
            {
                FormScopeViewPopulator.populateFormAndViewVariables(this.getContext(), form);
            }
        }
     ...
     * @return forward
     */
    protected String _sendPageVariable(final FormScopeFormImpl form, String forward)
    {
        //this method can be overridden
        return forward;
    }
     ...
     * @throws Throwable
     */
    private String _sendPageVariable(final FormScopeFormImpl form)
        throws Throwable
    {
        String forward = null;
        forward = "form-scope-a-page";
        forward = _sendPageVariable(form, forward);
        return forward;
    }
