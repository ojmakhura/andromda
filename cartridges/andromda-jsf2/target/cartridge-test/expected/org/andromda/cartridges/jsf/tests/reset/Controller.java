...
public abstract class Controller
    ...
    {
        // Resets all the "isSet" flags on the forms to false.
        this.getResetPageSendWithResetForm().resetIsSetFlags();
        this.getResetForm().resetIsSetFlags();
    }

    /**
     * Retrieves the {@link ResetPageSendWithResetFormImpl} form instance (normally you wont't
     * need to call this method explicitly, however this is here for times when you need to access the
     * <em>resetResetPageSendWithResetForm</em> instance outside of its assigned controller operation).
     * @return ResetPageSendWithResetFormImpl
     */
    protected ResetPageSendWithResetFormImpl getResetPageSendWithResetForm()
    {
        ...
        final UIForm uiForm = this.findForm(this.getContext().getViewRoot(), "resetResetPageSendWithResetForm");
        this.populateComponentInputs(uiForm);
        return (ResetPageSendWithResetFormImpl)this.resolveVariable("resetResetPageSendWithResetForm");
    }

    /**
     * 
     * This method is called when 'send with reset' is triggered in the view 'Reset page'.
     * It can be safely overridden in descendant classes.
ResetPageSendWithResetFormImpl)
     * @param form
     */
    protected void _resetPage_sendWithReset(ResetPageSendWithResetFormImpl form)
    {
        //this method can be overridden
    }

    /**
     * @return resetPageSendWithReset
     */
    public String resetPageSendWithReset()
    {
        return resetPageSendWithReset((FacesEvent)null);
    }

    /**
     * @param event
     */
    public void resetPageSendWithReset(final ActionEvent event)
    {
        this.resetPageSendWithReset((FacesEvent)event);
    }

    /**
     * @param event
     */
    public void resetPageSendWithReset(final ValueChangeEvent event)
    {
        this.resetPageSendWithReset((FacesEvent)event);
    }
    ...
    public String resetPageSendWithReset(final FacesEvent event)
    {
        String forward = null;
        final Object currentForm = this.resolveVariable("form");
        try
        {
            final ResetPageSendWithResetFormImpl form =
                this.getResetPageSendWithResetForm();
            form.setEvent(event);
            ...
            _resetPage_sendWithReset(form);

            forward = _secondResetAction(form);
            ...
            if (event != null)
            {
                ResetViewPopulator.populateFormAndViewVariables(this.getContext(), form);
            }
            form.reset();
        }
     ...
     * @return forward
     */
    protected String _secondResetAction(final ResetPageSendWithResetFormImpl form, String forward)
    {
        //this method can be overridden
        return forward;
    }
     ...
     * @throws Throwable
     */
    private String _secondResetAction(final ResetPageSendWithResetFormImpl form)
        throws Throwable
    {
        String forward = null;
        someOperation(form);
        forward = ((Controller)this.resolveVariable("controller")).reset();
        forward = _secondResetAction(form, forward);
        return forward;
    }

    /**
     * Retrieves the {@link ResetFormImpl} form instance (normally you wont't
     * need to call this method explicitly, however this is here for times when you need to access the
     * <em>resetResetForm</em> instance outside of its assigned controller operation).
     * @return ResetFormImpl
     */
    protected ResetFormImpl getResetForm()
    {
        // - we do this in the case a button that submitted the form was set to immediate (this should be removed
        //   when we found a better way to handle this).
        final UIForm uiForm = this.findForm(this.getContext().getViewRoot(), "resetResetForm");
        this.populateComponentInputs(uiForm);
        return (ResetFormImpl)this.resolveVariable("resetResetForm");
    }

    /**
     * This method is called when the use case 'Reset' starts.
     * It can be safely overridden in descendant classes.
ResetFormImpl)
     * @param form
     */
    protected void _reset_started(ResetFormImpl form)
    {
        //this method can be overridden
    }
    ...
    public String reset()
    {
        return reset((FacesEvent)null);
    }

    /**
     * @param event
     */
    public void reset(final ActionEvent event)
    {
        this.reset((FacesEvent)event);
    }

    /**
     * @param event
     */
    public void reset(final ValueChangeEvent event)
    {
        this.reset((FacesEvent)event);
    }
     ...
     */
    @SuppressWarnings({ "unchecked", "rawtypes" })
    public String reset(final FacesEvent event)
    {
        String forward = null;
        final Object currentForm = this.resolveVariable("form");
        try
        {
            final ResetFormImpl form =
                this.getResetForm();
            form.setEvent(event);
            this.setForm("form", form, true);
            // - pass any properties from the previous form along
            ...
            forward = _firstResetAction(form);
            final FacesMessage.Severity messageSeverity = this.getMaximumMessageSeverity();
            if (messageSeverity != null && FacesMessage.SEVERITY_ERROR.getOrdinal() <= messageSeverity.getOrdinal())
            {
                // - copy any messages to the 'currentForm'
     ...
     * @return forward
     */
    protected String _firstResetAction(final ResetFormImpl form, String forward)
    {
        //this method can be overridden
        return forward;
    }
     ...
     * @throws Throwable
     */
    private String _firstResetAction(final ResetFormImpl form)
        throws Throwable
    {
        String forward = null;
        forward = "reset-reset-page";
        forward = _firstResetAction(form, forward);
        return forward;
    }
