    ...
    extends QueryTranslator
{
    ...
    /**
     * Holds the arguments which have previously been used during translation.
     * The key is the argument name BEFORE translation, and the value is the
     * argument name AFTER translation.
     */
     ...
     * incrementing counter (currently just used for EJB-QL translation) --> may
     * want to find a cleaner way to do this.
     */
    protected String replaceCounterPattern(String fragment)
    {
        if (TranslationUtils.containsPattern(
            fragment,
            EjbQLTranslator.ARG_COUNTER))
        {
            fragment = TranslationUtils.replacePattern(
                fragment,
                EjbQLTranslator.ARG_COUNTER,
                this.getCounter());
        }
