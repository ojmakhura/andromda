...
public class LibraryTranslationFinder {
	...
	private LibraryTranslationFinder() {}
		...
		if (libraryTranslation == null) {
			...
			if (index == -1) {
				throw new IllegalArgumentException(methodName + 
					...
					+ "' must contain the character '" + libSeparator
			...
			String translationName = 
				translation.substring(index + 1, translationLength);
	
			if (library != null) {
				libraryTranslation = library.getLibraryTranslation(translationName);
				if (libraryTranslation == null) {
					logger.error("ERROR! no translation '" 
						+ translationName 
