...
class SpringMetafacadeUtils
{
    ...
    /**
     * Gets the remoting type for the passed in <code>classifier</code>. If
     * the remoting type can be retrieved from the <code>classifier</code>,
     * then that is used, otherwise the <code>defaultRemotingType</code> is
     * returned.
     * 
     * @return String the remoting type name.
     */
    static String getServiceRemotingType(
        ...
        String defaultServiceRemotingType)
    {
        ...
        {
            String remotingTypeValue = (String)classifier
                .findTaggedValue(SpringProfile.TAGGEDVALUE_SPRING_SERVICE_REMOTING_TYPE);
            // if the remoting type wasn't found, search all super classes
            if (StringUtils.isEmpty(remotingTypeValue))
            {
                remotingType = (String)CollectionUtils.find(classifier
                    .getAllGeneralizations(), new Predicate()
                {
                    public boolean evaluate(Object object)
                    {
                        return ((ModelElementFacade)object)
                            .findTaggedValue(SpringProfile.TAGGEDVALUE_SPRING_SERVICE_REMOTING_TYPE) != null;
                    }
                });
            }
    ...
    /**
     * Gets the remote service port for the passed in <code>classifier</code>.
     * If the remote service port can be retrieved from the
     * <code>classifier</code>, then that is used, otherwise the
     * <code>defaultRemoteServicePort</code> is returned.
     * 
     * @return String the remote service port.
     */
    static String getServiceRemotePort(
        ClassifierFacade classifier,
        String defaultRemoteServicePort)
    {
        ...
        if (classifier.hasStereotype(UMLProfile.STEREOTYPE_SERVICE))
        {
            String remoteServicePortValue = (String)classifier
                .findTaggedValue(SpringProfile.TAGGEDVALUE_SPRING_SERVICE_REMOTE_PORT);
            // if the remote service port wasn't found, search all super classes
            if (StringUtils.isEmpty(remoteServicePortValue))
            {
                remoteServicePort = (String)CollectionUtils.find(classifier
                    .getAllGeneralizations(), new Predicate()
                {
                    public boolean evaluate(Object object)
                    {
                        return ((ModelElementFacade)object)
                            .findTaggedValue(SpringProfile.TAGGEDVALUE_SPRING_SERVICE_REMOTE_PORT) != null;
                    }
                });
            }
