    ...
    implements Filter
{
        ...
        throws ServletException
    {
        ...
        try
        {
            ...
            if (form != null)
            {
                OneSubmitFormImpl formFieldsOneSubmitForm =
                    (OneSubmitFormImpl)variableResolver.resolveVariable(
                    facesContext,
                    "formFieldsOneSubmitForm");
                // - populate the formFieldsOneSubmitForm with any parameters from the previous form
                FormPopulator.populateForm(form, formFieldsOneSubmitForm, false);
            ...
            if (form != null)
            {
                final boolean selectableBackingListReadable = PropertyUtils.isReadable(form, "selectableBackingList");
                if (selectableBackingListReadable)
                {
                    Object backingList = PropertyUtils.getProperty(form, "selectableBackingList");
                    request.setAttribute("selectableBackingList", backingList);
                }
                final boolean multiSelectsBackingListReadable = PropertyUtils.isReadable(form, "multiSelectsBackingList");
                if (multiSelectsBackingListReadable)
                {
                    Object backingList = PropertyUtils.getProperty(form, "multiSelectsBackingList");
                    request.setAttribute("multiSelectsBackingList", backingList);
                }
            }
        }
