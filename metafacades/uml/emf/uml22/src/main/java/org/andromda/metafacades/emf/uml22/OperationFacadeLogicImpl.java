...
public class OperationFacadeLogicImpl
     ...
     * related elements from the Sequence Diagram or OCL Body in the UML model to
    ...
    private String getTypedArgumentList(final boolean withArgumentNames) {
        // TODO: Possible covariant of the method 'getTypedArgumentList' defined in the
        // class 'OperationFacadeLogic'
    ...
    protected Collection handleGetExceptions() {
        ...
        final Collection<DependencyFacade> ownerDependencies = new ArrayList<DependencyFacade>(
                this.getOwner().getSourceDependencies());
        ...
        final Collection<DependencyFacade> operationDependencies = new ArrayList<DependencyFacade>(
    ...
    protected boolean handleIsReturnTypePresent() {
        ...
        if (this.getReturnType() != null) {
            hasReturnType = !("void"
                    .equalsIgnoreCase(StringUtils.trimToEmpty(this.getReturnType().getFullyQualifiedName()))
                    || StringUtils.trimToEmpty(this.getReturnType().getFullyQualifiedName(true))
                            .equals(UMLProfile.VOID_TYPE_NAME));
        }
        if (LOGGER.isDebugEnabled()) {
            final String rtnFQN = this.getReturnType().getFullyQualifiedName(true);
            final boolean voidType = "void"
                    .equalsIgnoreCase(StringUtils.trimToEmpty(this.getReturnType().getFullyQualifiedName()));
            final String voidRtn = this.getReturnType().getFullyQualifiedName();
            LOGGER.debug("OperationFacadeLogicImpl.handleIsReturnTypePresent rtnFQN=" + rtnFQN + " voidType=" + voidType
                    + " voidRtn=" + voidRtn + " hasReturnType=" + hasReturnType);
        }
    ...
    protected String handleGetGetterSetterReturnTypeName() {
        ...
        if (returnType != null && (this.getUpper() > 1 || this.getUpper() == LiteralUnlimitedNatural.UNLIMITED)) {
            ...
            if (this.handleIsUnique()) {
                ...
                name = this.handleIsOrdered() ? mappings.getTo(UMLProfile.LIST_TYPE_NAME)
            ...
            if (BooleanUtils.toBoolean(
                 ...
                 * returnType.getAllSpecializations();
        ...
        if (name == null && returnType != null) {
            ...
            // TODO Apply to all primitive types, not just booleans. This is a special case
            // because of is/get Getters.
    ...
    /**
     * @return this.getUpper() > 1 || this.getUpper() ==
        ...
        CollectionUtils.filter(
                ...
                new Predicate() {
                    public boolean evaluate(final Object object) {
                        ...
                        return !param.getDirection().equals(ParameterDirectionKind.RETURN_LITERAL)
                                && !param.isException();
                    }
                });
        return arguments;
    ...
    /*
     * Override to change private to public, since it makes no sense to have private
     * operations in generated code
    ...
    protected OperationFacade handleGetOverriddenOperation() {
        ...
        while (overriddenOperation == null && ancestor != null) {
            for (final Iterator<OperationFacade> operationIterator = ancestor.getOperations()
                    .iterator(); overriddenOperation == null && operationIterator.hasNext();) {
